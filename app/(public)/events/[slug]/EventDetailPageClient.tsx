...
interface EventWithSections extends Event {
  sections: (EventSection & { files: EventSectionFile[] })[];
}

const getEventTypeLabel = (type: string | null, language: "ro" | "en") => {
  ...
  return types[language][type || "other"] || types[language]["other"];
};
...
export function EventDetailPageClient({ event }: EventDetailPageClientProps) {
  ...
  const labels = {
    backToEvents: language === "en" ? "Back to events" : "Înapoi la Evenimente",
    attachedDocuments: language === "en" ? "Attached documents" : "Documente atașate",
    interestedInEvent: language === "en" ? "Interested in this event?" : "Interesat de acest eveniment?",
    eventInfo: language === "en" 
    ...
    contactUs: language === "en" ? "Contact us" : "Contactează-ne",
  };
      ...
      <Card className="bg-primary/5 border-primary/20">
        <CardContent className="pt-6">
          <div className="text-center space-y-4">
            <h3 className="text-xl font-bold">{labels.interestedInEvent}</h3>
